...
class AccountSpecialityLink extends AppModel {
	...
	public function fetchTypes($id) {
		$sql = "SELECT account_speciality.* FROM {$this->table} INNER JOIN account_speciality ON account_speciality.id = {$this->table}.account_speciality_id WHERE {$this->table}.company_id = :id";
		$params[":id"] = $id;
		return $this->fetchAll($sql, $params);
	}
}